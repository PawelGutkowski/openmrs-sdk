...
public class PullRequestTest {
    ...
    public void setup() throws Exception{
        ...
        doReturn(Lists.newArrayList(commit1, commit2)).when(gitHelper)
                .getCommitDifferential(any(Git.class), any(Repository.class), anyString(), anyString());
        doReturn(Lists.newArrayList("test", issueId +" test")).when(gitHelper)
                .getCommitDifferentialMessages(any(Git.class), any(Repository.class), anyString(), anyString());
    ...
    public void should_workWithoutErrors() throws Exception {
        ...
        verify(gitHelper, atLeastOnce()).push(any(Git.class), eq(username), eq(password));
